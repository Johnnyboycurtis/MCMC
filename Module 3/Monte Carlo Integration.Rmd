...
Our first problem is dealing with bounds $(-\infty, x)$. We cannot direcly apply our MC integration algorithm directly because we do not know if x will be positive or negative, but a way around this issue is by breaking this problem into two scenarios. First, if $x > 0$, then we use the Monte Carlo estimator as is. However, if $x <  0$, then we esimate $\Phi(x)$ by $1 - \Phi(-x)$ 

I'll illustrate how this will be done.
