...
To continue with the above example, we'll first define our functions $h$, $f$, and $g$ to be 1, $N(0,1)$ and $Exp(1)$ truncated at 4. Our instrumental importance function $g$ will be of the form $p(x)/p(x \geq 4)$,
$$
\begin{aligned}
...
t = 4
X = rexp(N) + t
w = dnorm(X)/dexp(X - t) ## importance sampling weights normal/exp(x-4)

plot(cumsum(w) / 1:N, type = "l")
abline(a = pnorm(t, lower.tail = FALSE), b = 0, col = "blue")

print(pnorm(t, lower.tail = FALSE))
print(mean(w)) ## close results!
