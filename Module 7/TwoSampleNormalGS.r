...
par(mfrow=c(1,1))
#marginal posterior for each mu_j
  plot(density(mu.diff.GS), type="l", main="Posterior Density of Mean Difference (log-scale)", xlab=substitute(mu[b] - mu[a], list(a=levels(group)[1], b=levels(group)[2])), ylab="density")
  lines(HPDinterval(as.mcmc(mu.diff.GS)), c(0,0), col="red", cex=2, lty=3)  # add PI line
  legend("topright", legend="95% HPD interval", lty=2, col="red", cex=0.4)

  mean(mu.diff.GS > 0)  # Monte Carlo estimate of P(mu_normal > mu_low)
...
# Predictive densities for new observations (in the log-scale)

y.tilde.sample <- matrix(NA, nrow=net.iterations, ncol=2)
for(i in 1:net.iterations){
  for(j in 1:2){
  y.tilde.sample[i, j] <- rnorm(1, mean=mu.GS[i, j], sd=sigma.GS[i, j])
  }
}
par(mfrow=c(2, 1))
for(j in 1:2){
plot(density(y.tilde.sample[, j]), main=substitute(paste("Predictive Density in the ", a, " group"), list(a=levels(group)[j])), xlab="log(Diasorin Score)", ylab="Predictive Density", xlim=range(y.tilde.sample))
lines(HPDinterval(as.mcmc(y.tilde.sample[, j])), c(0,0), col="red", cex=2, lty=3)  # add Prediction Interval line
legend("topright", legend=c("95% Prediction Interval"), lty=2, col="red", cex=0.25)
}


# Predictive densities for new observations (in the original scale)
# Summary of the predictive distribution
par(mfrow=c(2, 1))
for(j in 1:2){
  plot(density(exp(y.tilde.sample[, j])), main=substitute(paste("Predictive Density in the ", a, " group"), list(a=levels(group)[j])), xlab="Diasorin Score", ylab="Predictive Density", xlim=c(0, 1000))
  lines(HPDinterval(as.mcmc(exp(y.tilde.sample[, j]))), c(0,0), col="red", cex=2, lty=3)  # add Prediction Interval line
  legend("topright", legend=c("95% Prediction Interval"), lty=2, col="red", cex=0.25)
}
summary.Bayes(y.tilde.sample)
...
# What is the predictive probability that a new diasorin score from
# the normal group exceeds a new score from the low group?

mean(exp(y.tilde.sample [, 1]) < exp(y.tilde.sample [, 2])) # MCMC estimate

# This is equivalent to

mean(y.tilde.sample [, 1] < y.tilde.sample [, 2])

# If we average the posterior means of mu_1 and mu_2, we get 5.1 (approximately)
# This value can be used as a cutoff point for classifying patients

# Consider the following classification rule: If a patient has a log diasoring score below 5.1, we
# we think that his/her bone turnover is low.

# Is this a good classification rule?

# What is the predictive probability of having a
# log diasorin value less than 5.1 for those in the low group?

mean(y.tilde.sample [, 1] < 5.1)

# What is the predictive probability of having a
# log diasorin value above 5.1 for those in the normal group?

mean(y.tilde.sample [, 2] > 5.1)

# The probability of misclassification is too high!
