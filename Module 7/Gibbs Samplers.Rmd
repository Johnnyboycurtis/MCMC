...
A changepoint model identifies a point (year) during the observation period (call it $k$) after which the 
...
$$
\begin{aligned}
p( \theta, \lambda, k | \mathbf{y} ) & \propto \left[\prod_{t=1851}^{k} \text{Poi}(y_i|\theta) \prod_{t=k+1}^{1962} \text{Poi}(y_i|\lambda) \right] \text{Gamma}(\theta|\alpha,\beta) \text{Gamma}(\lambda|\alpha, \beta) \frac{1}{111} \\
...
$$
\theta \sim \text{Gamma}(\sum_{t=1851}^{k} y_i +\alpha, k+\beta)
$$



$$
\lambda \sim \text{Gamma}(\sum_{t=k+1}^{1962} y_i + \alpha, 1962-k+\beta)
$$
...
$$
\displaystyle L(\mathbf{y} | \theta, \lambda, b_1, b_2) = e^{(\lambda - \theta)} (\frac{\theta}{\lambda})^{\sum_1^k y_i}
$$
