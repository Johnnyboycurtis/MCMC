...
or indirectly from a target distribution(s). 

- Applications of monte carlo methods are in hypothesis testing and Bayesian computation

- Monte Carlo methods well known for their applications in Bayesian methods

- Monte Carlo is not strictly Bayesian

- Monte Carlo is not strictly frequentist (Neyman-Pearson hypothesis testing)

- Parametric sampling (target distribution part of a family of distributions)

- Non-parametric sampling (functional form of target distribution not specified)

- MC methods also known as parametric Bootstrap

Monte Carlo methods are a method of statistical simulation. Simulation can help us understand reality without needing to run multiple experiments or costly calculations.


## Simulation: Gambler's ruin | Example of Simulation  { .selectable }
...
## Gambler's ruin, pt.2 { .selectable }
...
## Gambler's ruin, pt.3 { .selectable }
...
## Gambler's ruin, pt.4 { .selectable }
...
## Bayesian Example | Inference on a single proportion { .selectable }
...
We can conclude that the true probability lies between 53.00% and 59.05%, with median probability of 56.03%.
...
The previous example focused on direct simulation from a posterior distribution that was well-defined. However, there are some posteriors that will not be as easily identifiable. In general, Monte Carlo methods will prove useful for generating samples from *difficult to sample* target distributions.

The next section will delve into topics of random number generation from target distributions through direct methods and transformations.
