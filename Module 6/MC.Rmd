...
Consider a (discrete) Markov chain $(X_n)$ (with *n* iterations)  with two states, *[i, j]*. This chain takes $X_1, X_2, ...$ steps where at each step it transitions between the two states with some probability $p_{i,j}$. Let i and j takes values 0 and 1, respectively. Therefore, when the chain is at state $i = 0$, the chain can transition to state $j = 1$ with some probability, or remain at $i = 0$.
