...
## Pseu-random numbers  { .selectable }

One of the fundamental tools required in Monte Carlo methods is the ability to generate consistent *pseudo*-random variables from a specified probability distribution. One of the most fundamental number generation is the uniform random number generation
...
Random number generation for statistical purposes heavily relies on being able to obtain reliable samples from a Uniform distribution. There is a long history of work that has gone into this. Computers on their own can't generate *randomness*, but they can simulate what *random* may look like.
...
## Generation uniform samples, pt. 2 { .selectable }
...
- Distributions sometimes share relationships, and if the relationship is simple we can exploit it to generate random variables from a "simple to simulate" distribution and transform those random variables to generate samples from a "difficult to simulate" distribution. I'll give some examples below. 
...
## Example: Binomial trials { .selectable }

Another use of generating samples $U \sim U(0,1)$ is to generate Bernoulli and Binomial samples. For example, let $Y = \sum_{i = 0}^{N} X_i$ where $X ~ Bernoulli(p)$ such that $Y \sim Bin(n,p)$
