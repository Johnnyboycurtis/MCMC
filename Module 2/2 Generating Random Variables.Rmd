...
```{r}
cdf = c(0.0, 0.1, 0.3, 0.5, 0.7, 1.0) ## cumulative probabilities
results = numeric(length = 10000) ## creates a vector of zeros
u = runif(10000)
for(i in 2:6){
    ind = (cdf[i-1] < u) & (u <= cdf[i])
    results[ind] <- (i-2) ## returns an index for x vals
}
